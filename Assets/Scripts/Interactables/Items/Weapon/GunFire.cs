...
namespace Interactables.Items.Weapon
{
    public class GunFire : MonoBehaviour
    {
        ...
        {
            public GunFireRuntimeData(int bulletsFired, float reloadTimer, float fireRateCooldownTimer, bool isFireRateCoolingDown, bool isReloading)
            {
        ...
        bool ProjectileFire()
        {
            if(settings.weaponType != WeaponSettingsObject.WeaponType.ProjectileFire || runtimeData.isFireRateCoolingDown || runtimeData.isReloading) { return false; }
            Debug.Log("Fire");
        ...
        bool BurstFire()
        {
            if(settings.weaponType != WeaponSettingsObject.WeaponType.BurstFire || runtimeData.isFireRateCoolingDown || runtimeData.isReloading) return false;
        ...
        bool RaycastFire()
        {
            if (settings.weaponType != WeaponSettingsObject.WeaponType.RaycastFire || runtimeData.isFireRateCoolingDown || runtimeData.isReloading) { return false; }
            Debug.Log("Fire");
        ...
        {
            if(!runtimeData.isFireRateCoolingDown) { return; }
        ...
        void SetAmmoUI()
        {
            ammoUI.text = (settings.bulletsBeforeReload - runtimeData.bulletsFired).ToString() + "/" + settings.bulletsBeforeReload.ToString() ;
        }

        void SetAmmoBurstUI()
        {
            ammoUI.text = Mathf.Max(0, (1 - runtimeData.bulletsFired / settings.bulletsBeforeReload)).ToString() + "/" + 1.ToString();
        }
